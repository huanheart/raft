...
// 当前协程让出执行杄1�7
// 协程执行完成之后胡会自动yield,回到主协程，此时状��为TEAM
void Fiber::yield() {
  CondPanic(state_ == TERM || state_ == RUNNING, "state error");
...
void Fiber::MainFunc() {
  ...
  cur->cb_ = nullptr;
  cur->state_ = TERM;
  // 手动使得cur_fiber引用计数凄1�71
  auto raw_ptr = cur.get();
  cur.reset();
  // 协程结束，自动yield,回到主协稄1�7
  // 访问原始指针原因：reset后cur已经被释攄1�7
  raw_ptr->yield();
}

// 协程重置（复用已经结束的协程，复用其栈空间，创建新协程）
// TODO:暂时不允许Ready状��下的重罄1�7
